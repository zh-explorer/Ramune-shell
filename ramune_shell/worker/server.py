...
from ramune_shell.protocol import Request, TYPE_RQ, TYPE_FRAME, read_token
from ramune_shell.worker.dispatch import dispatch
...
class WorkerServer:
        ...
        try:
            ...
            if type_byte == TYPE_RQ:
                try:
                    ...
                    pass  # client closed or idle timeout, normal for pooled R/Q
                finally:
                    writer.close()
    ...
    _RQ_IDLE_TIMEOUT = 300.0  # close idle R/Q connections after 5 min
    ...
    async def _handle_rq(
        ...
        """Handle R/Q connection: loop request-response until closed or idle."""
        while True:
            req = await asyncio.wait_for(
                Request.async_read(reader), timeout=self._RQ_IDLE_TIMEOUT,
            )
